...
export default function () {
  ...
  var Spinner = {
    attach: function () {
      ...
      if (
        API.isInlineCheckout &&
        document.getElementById(Popup.inlineCheckoutContainerId)
      ) {
        document
          .getElementById(Popup.inlineCheckoutContainerId)
          .appendChild(this.spinSVG);
      } else {
  ...
  var API = {
    assignAttributeValues: function ($fscContainer) {
      ...
      this.beforeRequests = $fscContainer.getAttribute(
        "data-before-requests-callback",
      );
      this.afterRequests = $fscContainer.getAttribute(
        "data-after-requests-callback",
      );

      this.beforeMarkup = $fscContainer.getAttribute(
        "data-before-markup-callback",
      );
      this.afterMarkup = $fscContainer.getAttribute(
        "data-after-markup-callback",
      );

      this.decorateCallback = $fscContainer.getAttribute(
      ...
      this.validationCallback = $fscContainer.getAttribute(
        "data-validation-callback",
      );
      this.markupHelpers = $fscContainer.getAttribute(
        "data-markup-helpers-callback",
      );

      this.popupClosed = $fscContainer.getAttribute("data-popup-closed");
      this.popupWebhookReceived = $fscContainer.getAttribute(
        "data-popup-webhook-received",
      );
      this.popupEventReceived = $fscContainer.getAttribute(
        "data-popup-event-received",
      );
    ...
    init: function () {
      ...
      if (
        this.storefront.match(/pinhole/) !== null ||
        this.storefront.match(/popup/) !== null
      ) {
    ...
    continuousGet: function () {
      ...
      if (maybeSerial) {
        ...
        if (
          maybeSerial.hasOwnProperty("session") &&
          maybeSerial.hasOwnProperty("expires")
        ) {
    ...
    continuousStore: function (serial) {
      ...
      Helpers.storage.setItem(
        "fscSerial-" + this.storefront,
    ...
    updateSession: function () {
      ...
      var payload = {};
      var sessionRefreshId = Helpers.storage.getItem(
        "fscLatestSession-" + API.storefront,
      );
      var endpoint = sessionRefreshId
        ? "https://" +
          this.storefront +
          this.builder +
          "/refresh/" +
          sessionRefreshId
        : "https://" + this.storefront + this.builder + "/refresh/";
      this.chain({
        endpoint: endpoint,
        method: "GET",
        payload: payload,
    ...
    parseInput: function (input, callback) {
      ...
      if (input.hasOwnProperty("authenticate")) {
        ...
        if (input.authenticate.hasOwnProperty("key")) {
          this.authenticateUser(
            input.authenticate.payload,
            input.authenticate.key,
          );
        } else {
      ...
      if (input.hasOwnProperty("checkout")) {
        ...
        if (
          ...
          input.checkout.length > 10
        ) {
          Helpers.storage.setItem(
            "fscLatestSession-" + this.storefront,
            input.checkout,
          );
    ...
    syncInlineSession: function (responseURL, method, hasSelections) {
      try {
        if (
          this.isInlineCheckout &&
          responseURL &&
          responseURL.indexOf(this.builder) !== -1
        ) {
          ...
          if (inlineChekoutiFrame && inlineChekoutiFrame.contentWindow) {
            if (hasSelections) {
              this.checkoutRedirect({}, function (data, b) {
                inlineChekoutiFrame.contentWindow.postMessage(
                  { refreshUI: true },
                  "*",
                );
              });
            ...
            if (
              continuousSessionID ||
              (!continuousSessionID && method === "POST")
            ) {
              ...
              if (document.getElementById(Popup.inlineCheckoutContainerId)) {
                ...
                try {
                  ...
                  implicitLoaderInterval = setInterval(function () {
                    ...
                    var inlineContainerDIV = document.getElementById(
                      Popup.inlineCheckoutContainerId,
                    );
      ...
      data.groups.forEach(function (group) {
        ...
        group.items.forEach(function (item) {
          ...
          if (!item.selected) {
            ...
            if (group.type === "add") {
              group.selectableAdditions = true;
            }
          ...
          item.groups.forEach(function (group) {
            ...
            if (group.selectables === true) {
              item.selectables = true;
            }

            if (group.selectableReplacements === true) {
              item.selectableReplacements = true;
            }
    ...
    loadProducts: function (input, callback) {
      ...
      var expiresLocalStorage = Helpers.storage.getItem(
        "fscSerial-" + this.storefront,
      )
        ? JSON.parse(Helpers.storage.getItem("fscSerial-" + this.storefront))
      ...
      if (isEmpty(input) || expiresSBL) {
        payload = {};
        if (!this.sessionID) {
          //specific case which can use the optimized 'GET' request
          method = "GET";
        }
      } else {
        ...
        if (this.sblVersion.indexOf("sbl") !== -1) {
          ...
          sblVersionNumber = sblDirectoryPath.substring(
            sblDirectoryPath.indexOf("/") + 1,
            sblDirectoryPath.lastIndexOf("/"),
          );
        }
    ...
    checkoutRedirect: function (input, callback) {
      ...
      var sessionFinder = Helpers.storage.getItem(
        "fscLatestSession-" + API.storefront,
      );
    ...
    request: function (url, method, payload, skipsession, callback) {
      ...
      debug(
        " > > > Making request to " + url + " (" + method + ") with data:",
        sendPayload,
      );
      try {
    ...
    successHandler: function (req, callback, method) {
      ...
      if (
        responseData.hasOwnProperty("url") &&
        !responseData.hasOwnProperty("serial")
      ) {
    ...
    continueCheckoutWithURL: function (url, callback) {
      ...
      if (
        API.checkoutMode === "popup" &&
        (/\/popup-/.test(url) || /\/embedded-/.test(url))
      ) {
        if (fscCleanCheckout === true) {
          API.continuousReset();
        }
        Popup.drawFrame(url);
      } else {
  ...
  var Popup = {
    ...
    checkURL: function () {
      var findings = /fsc:invoke:(session|complete)/g.exec(
      ...
      if (
        findings &&
        findings.length > 0 &&
        (findings[1] === "session" || findings[1] === "complete")
      ) {
        lastSession = localStorage.getItem(
          "fscLatestSession-" + API.storefront,
        );
        ...
        if (lastSession) {
          ...
          var interval = setInterval(function () {
            if (document.readyState === "complete") {
              ...
              Popup.drawCanvas();
              Popup.drawFrame(
                "https://" + API.storefront + "/session/" + sessionPath,
              );
            }
          }, 100);
        }
    ...
    listener: function (event) {
      if (
        event.data.hasOwnProperty("fscPopupMessage") &&
        event.data.fscPopupMessage.hasOwnProperty("action")
      ) {
        ...
        if (a.action === "close") {
        ...
        } else if (a.action === "event" && a.hasOwnProperty("eventData")) {
          ...
          Popup.fsInlineCheckoutContainer =
            Popup.fsInlineCheckoutContainer ||
            document.getElementById(Popup.inlineCheckoutContainerId);
    ...
    drawCanvas: function () {
      ...
      if (!document.getElementById("fscCanvas") && !API.isInlineCheckout) {
        ...
        if (API.animateFader) {
          //TODO: get rid of these direct cross-module references
    ...
    drawFrame: function (url) {
      ...
      if (this.frame) {
        ...
        if (document.getElementById("fscCanvas")) {
          ...
          var fsInlineCheckoutContainer = document.getElementById(
            Popup.inlineCheckoutContainerId,
          );
          if (fsInlineCheckoutContainer) {
            ...
            error(
              "Make sure the inline checkout container is placed correctly",
            );
          }
        } else {
  ...
  var F = {
    ...
    update: function (path, quantity, callback) {
      F.push(
        { products: [{ path: path, quantity: parseInt(quantity, 10) }] },
        callback,
      );
    },
    ...
    recognize: function () {
      if (arguments.length > 0) {
        var callback =
          ...
          arguments[arguments.length].pop();
        var validKeys = [
          "firstName",
          "lastName",
          "email",
          ...
          if (values.hasOwnProperty(key)) {
            if (~validContactKeys.indexOf(key)) {
              ...
              for (var objKey in objValues) {
                if (
                  objValues.hasOwnProperty(objKey) &&
                  ~validAddressKeys.indexOf(objKey)
                ) {
  ...
  var Markup = {
    init: function () {
      ...
      this.listDriver = fscContainer
        .getAttribute("data-storefront")
        .split(".")[0];

      //are there any handlebars template on the page?
      this.$fscDynaContainers = document.querySelectorAll(
        "[data-fsc-items-container]",
      );
      this.dynaListObjs = [];
      if (typeof this.$fscDynaContainers[0] !== "undefined") {
        ...
        for (var i = 0; i < dynaLength; i++) {
          containerName = this.$fscDynaContainers[i].getAttribute(
            "data-fsc-items-container",
          );
          filterList =
            this.$fscDynaContainers[i].getAttribute("data-fsc-filter");
          this.dynaListObjs.push({
      ...
      for (var i = 0; i < this.dynaListObjs.length; i++) {
        if (this.dynaListObjs[i].containerName === templateId) {
          source = document.querySelector(
            "[data-fsc-template-for='" + templateId + "']",
          );
          if (source == null) {
            return;
          }
    ...
    process: function () {
      ...
      this.data.groups.forEach(function (group) {
        group.items.forEach(function (item) {
          if (
            ...
            item.description.hasOwnProperty("action") &&
      ...
      if (typeof window[API.beforeMarkup] === "function") {
        ...
        try {
          ...
          error(" --Error in data-before-markup-callback");
        }
      }
          ...
          function handlebarsCallback() {
            ...
            for (var i = 0; i < this.dynaListObjs.length; i++) {
              var source = document.querySelector(
                "[data-fsc-template-for='" +
                  this.dynaListObjs[i].containerName +
                  "']",
              );
              if (source !== null) {
                this.handlebarTemplates[i] = Handlebars.compile(
                  source.innerHTML,
                );
              }
            }
    ...
    addEventHandlers: function () {
      ...
      for (var i = 0; i < clickSelector.length; i++) {
        ...
        if (tag === "a") {
          ...
          tag === "select" ||
          tag === "textarea" ||
          (tag === "input" && changeTypes.indexOf(e.type.toLowerCase()) > -1)
        ) {
          if (e.addEventListener) {
    ...
    Update: function (elem) {
      ...
      if (
        ...
        // //get from source element quantity
          ...
          quantitySrcValue = quantityElement.getAttribute(
          ...
          if (quantityElement !== null) {
            ...
            switch (true) {
              case (tag === "input" &&
                quantityElement.type.toLowerCase() !== "checkbox") ||
                tag === "select":
                quantity = quantityElement.value;
                break;
              case tag === "input" &&
                quantityElement.type.toLowerCase() === "checkbox" &&
                !quantityElement.checked:
                quantity = 0;
                break;
              case tag === "input" &&
                quantityElement.type.toLowerCase() === "checkbox" &&
          ...
          if (
            elem.hasOwnProperty("selectedIndex") &&
            elem[elem.selectedIndex].value > -1
          ) {
    ...
    findValue: function (tag, ofitem) {
      ...
      if (!ofitem) {
        ...
        values = document.querySelectorAll(
          "[data-fsc-" +
            tag +
            "-value][data-fsc-item-path-value='" +
            ofitem +
            "']",
        );
      }
    ...
    parseAndPut: function (data) {
      ...
      this.processDynaOrderLists(
        retrievedItemsDyna,
        retrievedItemsUnaltered,
        retrievedDynaOrderLevel,
      );
    ...
    parseProductsDyna: function (data) {
      ...
      ProductGroup.prototype.setListType = function (driver) {
        if (driver === undefined) {
          ...
          if (
            driver === Markup.listDriver ||
            driver.indexOf(Markup.listDriver) > -1
          ) {
            this.listType = PRODUCT_GROUP_TYPES.storefront;
    ...
    parseOrderLevel: function (data) {
      ...
      for (var prop in data) {
        if (data.hasOwnProperty(prop) && prop !== "groups") {
          retrievedOrderItems = retrievedOrderItems.concat({
            key: prop,
            value: data[prop],
          });
        }
    ...
    processOrderLevelItems: function (retrievedOrderLevel) {
      ...
      if (!Array.isArray(retrievedOrderLevel)) {
        ...
        retrievedOrderLevel.forEach(function (orderLevelItem) {
          obj.processOrderLevelItem(
            orderLevelItem["key"],
            orderLevelItem["value"],
          );
        });
      }
    },
    ...
    processOrderLevelItem: function (property, propertyValue) {
      ...
      if (property === "taxExemptionAllowed") {
        var allTaxSmartDisplays = document.querySelectorAll(
          "[data-fsc-smartdisplay][data-fsc-order-taxid]",
        );

        Array.prototype.forEach.call(
          allTaxSmartDisplays,
          function (smartDisplayElement) {
            if (propertyValue) {
      ...
      if (property === "availablePaymentMethods") {
        var allPaypalSmartDisplays = document.querySelectorAll(
          "[data-fsc-smartdisplay][data-fsc-order-paypal-available]",
        );

        Array.prototype.forEach.call(
          allPaypalSmartDisplays,
          function (smartDisplayElement) {
      ...
      if (property === "selections") {
        ...
        if (allControlableItems !== null) {
          for (i = 0; i < allControlableItems.length; i++) {
            ...
            if (
              e.hasAttribute("data-fsc-selections-smartselect") &&
              e.tagName === "INPUT" &&
      ...
      for (i = 0; i < elements.length; i++) {
        obj.processPropToElem(
          property,
          propertyValue,
          elements[i],
          "data-fsc-order-" + property,
        );
      }
    },
    /**
     ...
     * @memberOf Markup
     */
    processDynaOrderLists: function (
      retrievedItems,
      retrievedItemsUnaltered,
      retrievedOrderLevel,
    ) {
        ...
        for (i = 0; i < dynaLength; i++) {
          for (var j = 0; j < retrievedLength; j++) {
            if (this.dynaListObjs[i].filterList !== null) {
              ...
              tempRetrievedItems[j].items = this.applyDynaFilter(
                tempRetrievedItems[j].items,
                this.dynaListObjs[i],
              );
            }
          }
    ...
    futureAddProps: function (productData) {
      ...
      if (productData.future.intervalUnit === "week") {
        endsDate.setDate(
          endsDate.getDate() +
            productData.future.discountDurationLength *
              productData.future.intervalLength *
              7,
        );
      }
    ...
    processProduct: function (productData, productPath) {
      ...
      var allRecognizedProducts = document.querySelectorAll(
        "[data-fsc-item-path='" + productPath + "']",
      );
      ...
      if (allRecognizedProducts !== null) {
        for (i = 0; i < allRecognizedProducts.length; i++) {
          ...
          if (
            e.hasAttribute("data-fsc-item-selection-smartselect") &&
            e.tagName === "INPUT" &&
            (e.type.toLowerCase() === "checkbox" ||
              e.type.toLowerCase() === "radio")
          ) {
              ...
              for (var innerprop in productData[prop]) {
                if (productData[prop].hasOwnProperty(innerprop)) {
                  ...
                  for (i = 0; i < elemList.length; i++) {
                    Markup.processPropToElem(
                      innerprop,
                      productData[prop][innerprop],
                      elemList[i],
                      "data-fsc-item-" + prop + "-" + innerprop,
                      sDisplay,
                    );
                  }
                }
        ...
        debug(
          "no markup to markup for the product-path and/or no productData property exist",
        );
      }
    },
    ...
    processPropToElem: function (
      ...
      debug(
        "Processing values for a single element:",
        property,
        elem,
        smartDisplay,
      );
      ...
      if (tag === "a") {
        ...
        if (elem[propertyValue - 1]) {
          elem[propertyValue - 1].setAttribute("selected", true);
        }
      } else if (tag === "button") {
        debug("Setting value to", propertyValue, elem);
        elem.value = propertyValue;
      ...
      if (
        typeof elem.getAttribute("data-fsc-smartdisplay") !== "undefined" &&
        elem.getAttribute("data-fsc-smartdisplay") !== null &&
        smartDisplayProperties.indexOf(original) > -1
      ) {
        ...
        if (elem.getAttribute("data-fsc-smartdisplay") === "") {
          ...
          elemToApply = document.querySelector(
            "[" + elem.getAttribute("data-fsc-smartdisplay") + "]",
          );
        }
      ...
      if (
        elem.hasAttribute("data-fsc-callback") &&
        typeof window[elem.getAttribute("data-fsc-callback")] === "function"
      ) {
        debug(
          "Agreed to run CallBack function for",
          property,
          elem.getAttribute("data-fsc-callback"),
        );
        window[elem.setAttribute("data-fsc-callback")](
          elem,
          property,
          propertyValue,
        );
    ...
    displayDynaOrderLists: function (orderItems, template, containerName) {
      // clear out first
      var itemCart = document.querySelector(
        "[data-fsc-items-container='" + containerName + "']",
      );
  ...
  Object.defineProperty(window, "_f", {
    ...
    get: function () {
      console.log(
        "Warning: '_f' is deprecated, please use 'fastspring.builder' instead",
      );
      return F;
    },
  });
